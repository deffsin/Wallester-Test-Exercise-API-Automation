...
from helpers.api_helpers import post_to_verify_login_with_valid_details
...
def test_post_to_verify_login_with_valid_details(api_client):
    email = "deffsinco@gmail.com"
    password = "qwerty"

    response_data = post_to_verify_login_with_valid_details(
        api_client, email=email, password=password, expected_status_code=200
    )
    ...
    logger.info(f"POST to verifyLogin response: {response_data}")
